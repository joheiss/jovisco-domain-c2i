...
export class InvoiceItems extends TransactionItems<InvoiceItem, InvoiceItemData> {
    ...
    get totalCashDiscountAmount(): number {
        return this._items.reduce((sum, item) => sum + item.cashDiscountValue, 0);
    }

    get totalCashDiscountBaseAmount(): number {
        return this._items.reduce((sum, item) => sum + item.discountableValue, 0);
    }

    get totalDiscountedNetValue(): number {
        return this._items.reduce((sum, item) => sum + item.discountedNetValue, 0);
    }
    ...
    get totalNetValue(): number {
        return this._items.reduce((sum, item) => sum + item.netValue, 0);
    }

    get totalPaymentAmount(): number {
        return this._items.reduce((sum, item) => sum + item.discountedValue, 0);
    }

    get totalVatAmount(): number {
        return this._items.reduce((sum, item) => sum + item.vatValue, 0);
    }
}